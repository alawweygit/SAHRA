...
const GUIDANCE = {
  ...
  mostlikely:   '"Who is most likely to…" prompts that spark funny debates. LENGTH: one short, punchy sentence, under 12 words — never a run-on setup or mini-story (e.g. "Who is most likely to fake a language app streak?" not "Who is most likely to download Duolingo before a trip, use it for one day, and then just point at things and smile the whole vacation?"). LANGUAGE: simple, everyday words a 12-year-old would understand — no complicated phrasing, the humor should come from the IDEA, not clever wording. FUNNY: the scenario itself should genuinely make people laugh or go "haha, that\'s so [name]" — pick a specific, relatable, slightly embarrassing or chaotic behavior, not a generic or bland one. MIX THE STYLE: within a batch, blend two flavors roughly evenly — everyday-embarrassing (e.g. "Who is most likely to text their ex by accident?", "Who is most likely to forget their own birthday?") and slightly absurd/chaotic (e.g. "Who is most likely to accidentally join a cult?", "Who is most likely to get kicked out of a wedding?") — do not make every item the same flavor. DIVERSITY: cover clearly different situations and topics across the batch (travel, relationships, everyday habits, social settings, etc.) — never repeat the same underlying scenario or joke reworded, and never reuse the exact examples given here verbatim. AVOID: multi-clause sentences, unnecessary setup/backstory, and anything that reads like a written paragraph instead of something you\'d blurt out loud to friends.',
