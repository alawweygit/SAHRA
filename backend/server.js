...
const GUIDANCE = {
  ...
  blendin:      'PUSH-THE-BUTTON-STYLE SOCIAL WRITING PROMPTS, NEVER TRIVIA. Each object contains two short, closely related everyday prompts: "a" goes to most players and "b" goes to one odd player. THE REAL TEST IS ANSWER OVERLAP, NOT WORDING SIMILARITY: what matters is not how many words differ between "a" and "b" — it is whether a typical honest answer to "a" could also pass as a normal answer to "b", and vice versa, often enough that the odd player\'s answer doesn\'t immediately stick out. Before finalizing a pair, mentally answer both prompts yourself: if the two answer-spaces feel completely disconnected (nothing you\'d say to one makes sense for the other), the pair has failed even if the wording only changed slightly — and conversely, a pair can survive a bigger conceptual jump as long as the actual answers still plausibly overlap. Good pair (different concepts, but real answers overlap heavily): "Something you\'d pack for school" / "Something you\'d pack for work" — a pen, a water bottle, headphones, a charger could honestly answer either one. Bad pair (looks like a small wording change, but answers never overlap): "Something you say to your mom" / "Something you say to your boss" — almost nothing a normal person says to their mother ("love you", "I\'m hungry") would ever be said to a boss, so the odd one out is instantly obvious even though only one word changed. ESCALATING DIFFICULTY ACROSS A BATCH: mirror the real game\'s pacing — make the FIRST pair in a batch the closest, most-overlapping pair (hardest to spot the odd player), and let LATER pairs in the same batch diverge a bit more (still connected, but a little easier to catch), building suspicion as the round progresses rather than every pair being equally obvious or equally subtle. Every person must understand both prompts instantly without needing factual, scientific, historical, sports, geographic, celebrity, or specialist knowledge. NEVER ask for a correct fact. NEVER use a question mark. NEVER begin with What, Which, Who, Where, When, How many, Name the, or their Arabic trivia equivalents. Keep each prompt under 12 words, natural, concrete, social, and party-appropriate. Do not copy the examples verbatim; invent fresh pairs across daily life, relationships, school/work, food, travel, home, celebrations, and common habits, always checking the answer-overlap test above before finalizing.',
