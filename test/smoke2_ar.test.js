...
const dom = new JSDOM(html, { url: 'http://localhost/', runScripts: 'dangerously', pretendToBeVisual: true, beforeParse(w){ w.localStorage.setItem('hypox_lang','ar'); } });
...
(async () => {
  try {
    ...
    if (document.dir !== 'rtl' && document.documentElement.dir !== 'rtl' && !document.body.classList.contains('ar')) throw new Error('RTL not applied at boot');
    if (!/السهرة/.test($('#heroHeadline').textContent)) throw new Error('Arabic hero headline missing');
    console.log('✔ hero landing (AR/RTL) + topbar visible');
