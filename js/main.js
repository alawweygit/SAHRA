...
(() => {
  ...
  function buildTitleScreen(){
    ...
    if(hstart){hstart.textContent=LANG==='ar'?'▶ ابدأ لعبة':'START A GAME ▶';hstart.onclick=()=>{Audio_.sfx.pop();$('#roundPill').textContent=LANG==='ar'?'اختر لعبة':'PICK A GAME';$('#roundPill').style.visibility='visible';$('#topbar').classList.add('show');document.getElementById('topbarBack')?.style.setProperty('visibility','hidden');show('#scr-games');};}
    const hstart2=$('#heroStart2');
    if(hstart2){hstart2.textContent=LANG==='ar'?'▶ ابدأ لعبة':'START A GAME ▶';hstart2.onclick=()=>{Audio_.sfx.pop();$('#roundPill').textContent=LANG==='ar'?'اختر لعبة':'PICK A GAME';$('#roundPill').style.visibility='visible';$('#topbar').classList.add('show');document.getElementById('topbarBack')?.style.setProperty('visibility','hidden');show('#scr-games');};}
